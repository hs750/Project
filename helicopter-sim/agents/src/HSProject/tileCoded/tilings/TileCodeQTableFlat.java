...
/**
 * An implementation of the {@link TileCodeQTableInterface} with a flatter data
 * structure. Slower than {@link TileCodeQTable} when many state actions are
 * visited.
 * 
 * @deprecated
 * @author harrison
...
public class TileCodeQTableFlat implements TileCodeQTableInterface {
	...
	public void put(Tile state, Tile action, double value, Action actualAction) {
		...
		if (newAction) {
			actionsForStates.put(state.value_, actions);// only need to put the
														// new actions if it is
														// new, otherwise the
														// byref nature means it
														// is already in.
		}
		// System.out.println(actionsForStates.size() + " " + qTable.size() + "
		// " + actions.size());
	}
