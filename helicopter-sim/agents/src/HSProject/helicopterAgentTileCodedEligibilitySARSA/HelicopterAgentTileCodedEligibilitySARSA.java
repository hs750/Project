package HSProject.helicopterAgentTileCodedEligibilitySARSA;
...
public class HelicopterAgentTileCodedEligibilitySARSA implements AgentInterface {
		...
		if(!exploringFrozen){
    	    ...
    	    for( int i=0; i<numStateTilings; i++ ) {
    	    	for(int j = 0; j < numActionTilings; j++){
    	    		...
    	    		qTable.eligibilityUpdate(curStates[i], actions[j], delta, action);
		...
		if(!exploringFrozen){
    	    ...
    	    for( int i=0; i<numStateTilings; i++ ) {
    	    	for(int j = 0; j < numActionTilings; j++){
    	    		...
    	    		qTable.eligibilityUpdate(curStates[i], actions[j], delta, lastAction);
   ...
   private Action egreedy(Observation theState) {
	   ...
	   if(maxAction.getAction() == null){
		   if(exploringFrozen){
			   System.out.println("using random on test case");
		   }
		   return randomAction();
	   }
