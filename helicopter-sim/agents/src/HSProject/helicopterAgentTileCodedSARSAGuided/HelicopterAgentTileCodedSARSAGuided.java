package HSProject.helicopterAgentTileCodedSARSAGuided;
...
public class HelicopterAgentTileCodedSARSAGuided implements AgentInterface {
	...
	public Action agent_step(double reward, Observation o) {
		...
		if(!exploringFrozen){
    	    ...
    	    for( int i=0; i<numStateTilings; i++ ) {
    	    	for(int j = 0; j < numActionTilings; j++){
    	    		double curQ  = qTable.getQValue(curStates[i], actions[j]);
    	    		double val   = curQ + (( alpha * (reward + (gamma*newQ[i][j]) - curQ)) / (double)(numStateTilings * numActionTilings));   	    		
	...
	protected Action randomAction(Observation o){
		...
		agent_policy(o, a);
		a.doubleArray[0] += randGenerator.nextGaussian()/4.0;
		a.doubleArray[1] += randGenerator.nextGaussian()/4.0;
		a.doubleArray[2] += randGenerator.nextGaussian()/4.0;
		a.doubleArray[3] += randGenerator.nextGaussian()/4.0;
